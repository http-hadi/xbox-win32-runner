...
// ===========================================================================
  ...
  // UWP MSBuild targets re-define WIN32_LEAN_AND_MEAN on the command line,
  // which causes <windows.h> to skip <winuser.h>, <wingdi.h>, <mmsystem.h>,
  // <winsvc.h>, <shlobj.h>, etc. We undef it here and re-include the
  // sub-headers that were skipped. This is the ONLY reliable way to get
  // the full Win32 type set in a UWP project.
  #undef WIN32_LEAN_AND_MEAN
  #undef NOGDI
  #undef NOUSER
  #undef NOSERVICE
  ...
  // Re-include sub-headers now that exclusion macros are undef'd.
  // These are all guarded by include guards so they're no-ops if already
  // fully included. But with WIN32_LEAN_AND_MEAN they were only PARTIALLY
  // included (guards set but content skipped), so we need to force them.
  #ifdef _WINUSER_
  #undef _WINUSER_
  #endif
  #include <winuser.h>
